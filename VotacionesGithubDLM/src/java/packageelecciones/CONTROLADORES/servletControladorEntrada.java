 ...
 * @author pc
 */
@WebServlet(name = "servletControladorEntrada_1", urlPatterns = {"/servletControladorEntrada_1"})
public class servletControladorEntrada extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int aux;
        try {
            aux = opera.darAlta(Conexion, vota);
        } catch (SQLException ex) {
            aux = 2;
        }
        HttpSession sesion = request.getSession(true);
        sesion.setAttribute("altas", aux);
        response.sendRedirect("VISTAS/Altas.jsp");
    }
